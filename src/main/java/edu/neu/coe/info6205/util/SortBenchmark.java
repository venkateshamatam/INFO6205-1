...
public class SortBenchmark {
    ...
    public void sortLocalDateTimes(final int n, Config config) throws IOException {
        ...
        // CONSIDER finding the common ground amongst these sorts and get them all
        // working together.

        // NOTE Test on date using pure tim sort.
        if (isConfigBenchmarkDateSorter("timsort"))
            logger.info(benchmarkFactory("Sort LocalDateTimes using Arrays::sort (TimSort)", Arrays::sort, null)
                    .runFromSupplier(localDateTimeSupplier, 100) + "ms");

        // NOTE this is supposed to match the previous benchmark run exactly. I don't
        // understand why it takes rather less time.
        if (isConfigBenchmarkDateSorter("timsort")) {
            logger.info(benchmarkFactory("Repeat Sort LocalDateTimes using timSort::mutatingSort",
                    new TimSort<>(helper)::mutatingSort, null).runFromSupplier(localDateTimeSupplier, 100) + "ms");
            // NOTE this is intended to replace the run two lines previous. It should take
    ...
    void benchmarkStringSorters(String[] words, int nWords, int nRuns) {
        logger.info(
                "Testing pure sorts with " + formatWhole(nRuns) + " runs of sorting " + formatWhole(nWords) + " words");
        ...
        if (isConfigBenchmarkStringSorter("mergesort")) {
            ...
            // modified this to run pure sorts.
        }
        ...
        // NOTE: this is very slow of course, so recommendation is not to enable this
        // option.
    ...
    void benchmarkStringSortersInstrumented(String[] words, int nWords, int nRuns) {
        logger.info("Testing with " + formatWhole(nRuns) + " runs of sorting " + formatWhole(nWords) + " words"
                + (config.isInstrumented() ? " and instrumented" : ""));
        Random random = new Random();
        ...
        if (isConfigBenchmarkStringSorter("mergesort")) {
            ...
            // modified to run pure sorts
            runMergeSortBenchmark(words, nWords, nRuns, true, true, helper);
    ...
    private void sortIntegersByShellSort(final int n) {
        ...
        final double t2 = new Benchmark_Timer<Integer[]>(
                ...
                (xs) -> Arrays.copyOf(xs, xs.length),
                Arrays::sort,
                null).runFromSupplier(integersSupplier, 100);
        for (TimeLogger timeLogger : timeLoggersLinearithmic)
            timeLogger.log(t2, n);
    }

    // This was added by a Student. Need to figure out what to do with it. What's
    // different from the method with int parameter??
    ...
    private void sortStrings(Stream<Integer> wordCounts) {
        ...
        // SortBenchmark::lineAsList), config.getInt("benchmarkstringsorters", "words",
        // 1000), config.getInt("benchmarkstringsorters", "runs", 1000));
     ...
     * @param sorter       the sorter to use--NOTE that this sorter will be closed
     *                     at the end of this method.
     * @param preProcessor the pre-processor function, if any.
     * @param timeLoggers  a set of timeLoggers to be used.
     */
    static void runStringSortBenchmark(String[] words, int nWords, int nRuns, SortWithHelper<String> sorter,
            UnaryOperator<String[]> preProcessor, TimeLogger[] timeLoggers) {
        new SorterBenchmark<>(String.class, preProcessor, sorter, words, nRuns, timeLoggers).run(nWords);
        sorter.close();
    }

    /**
     * Method to run a sorting benchmark using the standard preProcess method of the
     * sorter.
     *
     * @param words       an array of available words (to be chosen randomly).
     * @param nWords      the number of words to be sorted.
     ...
     * @param timeLoggers a set of timeLoggers to be used.
     *                    <p>
     *                    NOTE: this method is public because it is referenced in a
     *                    unit test of a different package
     */
    public static void runStringSortBenchmark(String[] words, int nWords, int nRuns, SortWithHelper<String> sorter,
     ...
     * @param timeLoggers  a set of timeLoggers to be used.
     */
    static void runIntegerSortBenchmark(Integer[] numbers, int n, int nRuns, SortWithHelper<Integer> sorter,
            UnaryOperator<Integer[]> preProcessor, TimeLogger[] timeLoggers) {
        new SorterBenchmark<>(Integer.class, preProcessor, sorter, numbers, nRuns, timeLoggers).run(n);
        sorter.close();
    }

    /**
     * For mergesort, the number of array accesses is actually 6 times the number of
     * comparisons.
     * That's because, in addition to each comparison, there will be approximately
     * two copy operations.
     ...
     * @return the minimum number of comparisons possible to sort n randomly ordered
     *         elements.
     */
     ...
     * elements.
     * For insertion sort, each (low-level) swap fixes one inversion, so on average
     * there are this number of swaps.
    ...
    private static Benchmark<LocalDateTime[]> benchmarkFactory(String description, Consumer<LocalDateTime[]> sorter,
            Consumer<LocalDateTime[]> checker) {
    ...
    private static void doPureBenchmark(String[] words, int nWords, int nRuns, Random random,
        ...
        // CONSIDER we should manage the space returned by fillRandomArray and
        // deallocate it after use.
        final double time = benchmark.runFromSupplier(
                () -> Utilities.fillRandomArray(String.class, random, nWords, r -> words[r.nextInt(words.length)]),
                nRuns);
        for (TimeLogger timeLogger : timeLoggersLinearithmic)
    ...
    // 100) + "ms");
    // // NOTE: this is intended to replace the run in the previous line. It should
    // take the exact same amount of time.
    // runDateTimeSortBenchmark(LocalDateTime.class, localDateTimes, 100000, 100,
    // i);
    // }
    ...
    private void runMergeSortBenchmark(String[] words, int nWords, int nRuns, Boolean insurance, Boolean noCopy,
        ...
        Config x = config.copy(MergeSort.MERGESORT, MergeSort.INSURANCE, insurance.toString()).copy(MergeSort.MERGESORT,
                MergeSort.NOCOPY, noCopy.toString());
    ...
    private void runDateTimeSortBenchmark(Class<?> tClass, ChronoLocalDateTime<?>[] dateTimes, int N, int m)
            throws IOException {
        final SortWithHelper<ChronoLocalDateTime<?>> sorter = new TimSort<>();
        @SuppressWarnings("unchecked")
        final SorterBenchmark<ChronoLocalDateTime<?>> sorterBenchmark = new SorterBenchmark<>(
                (Class<ChronoLocalDateTime<?>>) tClass, (xs) -> Arrays.copyOf(xs, xs.length), sorter, dateTimes, m,
                timeLoggersLinearithmic);
        sorterBenchmark.run(N);
    }

    /**
     * For (basic) insertionsort, the number of array accesses is actually 6 times
     * the number of comparisons.
     * That's because, for each inversion, there will typically be one swap (four
     * array accesses) and (at least) one comparison (two array accesses).
     * Thus, in the case where comparisons are based on primitives,
     * the normalized time per run should approximate the time for one array access.
     */
